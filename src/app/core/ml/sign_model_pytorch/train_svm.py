...
-------
  ...
  • sos / call                       — SYNTHETIC landmarks (no webcam needed)
...
_SYNTHETIC_SIGNS: dict[str, np.ndarray] = {
    ...
    # In our coord system y is NEGATIVE upward, so thumb-down = large POSITIVE y.
    # Opposite of EMERGENCY (thumbs-up). Zero overlap with any A-Z ASL letter.
    ...
    # Key distinction from ASL F: in F the 3 remaining fingers point UP.
    # Here they curl DOWN into the palm — completely different landmark pattern.
    # Thumb tip and index tip meet to form a visible O shape.
    "ok_sign": np.array(
        [
            [0.00, 0.00, 0.000],  # wrist
            [0.42, -0.12, 0.020],  # thumb CMC
            ...
            [0.44, -0.72, 0.040],  # index TIP  (meeting thumb tip — same coords)
            [0.05, -1.00, 0.000],  # middle MCP
...
def main() -> None:
    ...
    if args.test:
        print("QUICK TEST MODE — 3 ASL classes + all 5 emergency signs")
